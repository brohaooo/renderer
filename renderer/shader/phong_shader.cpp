...
{
	vec4 temp_vert	 = to_vec4(payload.model->vert(nfaces, nvertex), 1.0f);//读入的顶点
	vec4 temp_normal = to_vec4(payload.model->normal(nfaces, nvertex), 1.0f);//读入的法向量
	// printf("!!!!! %f %f %f %f\n",temp_vert[0],temp_vert[1],temp_vert[2],temp_vert[3]);
	payload.uv_attri[nvertex]		 = payload.model->uv(nfaces, nvertex);
	payload.homo_clipping_uv[nvertex]			 = payload.uv_attri[nvertex];
	payload.clipcoord_attri[nvertex] = payload.mvp_matrix * temp_vert;
	payload.homo_clipping_clipcoord[nvertex]    = payload.clipcoord_attri[nvertex];

	// printf("!!!!! %f %f %f %f\n",payload.homo_clipping_clipcoord[nvertex][0],payload.homo_clipping_clipcoord[nvertex][1],payload.homo_clipping_clipcoord[nvertex][2],payload.homo_clipping_clipcoord[nvertex][3]);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		payload.homo_clipping_worldcoord[nvertex][i]		= temp_vert[i];
		payload.normal_attri[nvertex][i]	    = temp_normal[i];
		payload.homo_clipping_normal[nvertex][i]		    = temp_normal[i];
	}
}
...
vec3 PhongShader::fragment_shader(float alpha, float beta, float gamma)
{
	...
	ka = (ka*payload.ka_ratio)/50.0;
	vec3 kd = payload.model->diffuse(uv);
	kd = (kd*payload.kd_ratio)/50.0;
	vec3 ks(0.8, 0.8, 0.8);
	ks = (ks*payload.ks_ratio)/50.0;

	// set light information
	float p = 150.0;
	vec3 l = unit_vector(vec3(5, 20, -20));//光源位置
	...
	ambient = cwise_product(ka, light_ambient_intensity) ;//环境自带光
	diffuse = cwise_product(kd, light_diffuse_intensity) * float_max(0, dot(l, normal));//光照漫反射
	specular = cwise_product(ks, light_specular_intensity) * float_max(0, pow(dot(normal, h), p));//镜面反射
